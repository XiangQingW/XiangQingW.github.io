...
## 什么是飞行脚本？  
### 回答 1
Flyway脚本是一个文件，您可以将SQL命令放在数据库中以进行迁移或版本控制。  
...
实际上，网页上的HTML / CSS / JS是程序中内置脚本解释器的示例。如果它没有这样的脚本能力想象中，您需要将每个网页预先编译到所有可能的客户端PC上的机器代码...  
脚本将始终具有其用途。倾向于说，当它不作为尽可能快地运行并尽可能少的资源使用时，脚本是一个很好的契合。从另一边：编译时会在使用中创造难以克服的问题，然后脚本是最明显的选择。  
### 回答 3
选项不仅适用于剧本，而且适用于大多数文学属性 - 播放，小说，文章，真正的生活博物馆  
...
NMS.  
### 回答 4
取决于你问谁;一般来说，那些说他们完全不同的人在编程前面有一个隐含的真实。  
编程仅意味着在表单中编写计算机的指令（通过各种其他程序）可以运行自己。无论是编译还是解释，无论是存储在Web上，存储在磁盘上，存储为固件，或者键入内存中的短信（要粘在一点），如Bash子程序或旧的8位交互式基本......或者只是为了跑一次，就像一个衬里一边的衬里）这是所有编程，而一些程序员可能会更好地获得报酬，而没有比任何其他程序更真实的编程形式。  
...
### 回答 5
从根本上  
脚本基本上是一个说明列表，用于告诉另一个程序该做什么。例如，Perl或Python是一种脚本语言，旨在告诉终端程序或命令解释器该怎么做，什么程序执行，并且在没有许多仪式或语法开销的情况下执行它。另一个例子是JavaScript，这是一种脚本语言，可以告诉Web浏览器该做什么。然后是emacs lisp，这是一种脚本语言，旨在告诉Emacs编辑器该怎么做。脚本通常不涉及非常重的编程，因为他们应该是轻盈且易于鞭打。  
尽管如此，脚本语言已被用于编写非常复杂的软件。最大的Python程序是约250k的代码行。一些JavaScript应用程序是数千行长。是的，这些是脚本！  
### 回答 6
它并不完全清楚这个问题的上下文是什么：'脚本是什么意思？  
根据用法，可以是：  
a）处方  
b）包含演员行的文件  
c）行动计划  
d）手稿  
e）非正式的计算机程序  
由于A-D都很无聊，我回答e）。在此上下文中，脚本是一种计算机程序。没有正式的定义，将脚本，应用程序，应用程序和程序分隔在此上下文中 - 您可以大多数时间使用这些术语互换。但是当您将程序称为脚本时，您可以使用一些假设  
它并不完全清楚这个问题的上下文是什么：'脚本是什么意思？  
根据用法，可以是：  
a）处方  
b）包含演员行的文件  
...
e）非正式的计算机程序  
由于A-D都很无聊，我回答e）。在此上下文中，脚本是一种计算机程序。没有正式的定义，将脚本，应用程序，应用程序和程序分隔在此上下文中 - 您可以大多数时间使用这些术语互换。但是当您将程序称为脚本时，您可以使用一些假设：  
该计划可能没有编译。作为脚本的程序通常用解释器执行，如PHP，Ruby，Python，Perl，或者只是您所在的shell。  
该程序可能没有正式定义。 Systems Administrators一直编写脚本，以完成不需要大量预见的简单任务。脚本也用于一次性任务，例如对文件系统进行搜索以查找特定的信息，例如：  
寻找 。 -type f -exec grep -il'致命错误'|排序| Uniq -c |排序-n.  
该单衬班将在当前目录下的文件系统中搜索文件系统，并生成包含字符串“致命错误”的每个文件名的列表;如果文件系统的多个位置存在相同的文件名，则将显示在列表的底部。这是一个自定义搜索，我不太可能在很快再次做到，所以而不是坐下来提出这个程序的正式设计，我只是把它扔在一起。  
### 回答 7
脚本或脚本语言是一种用于特殊运行时环境的编程语言，用于自动执行任务的执行，任务可以替代地由人工操作员执行。脚本语言通常被解释（而不是编译）。  
基本上，所有脚本语言都是编程语言。两者之间的理论差异是脚本语言不需要编译步骤并且是相当解释的。例如，通常，需要在运行之前编译C程序，而正常情况下，不需要编译像JavaScript或PHP这样的脚本语言。  
通常，编译程序运行比解释程序更快，因为它们是首次转换本机机代码。此外，编译器只读取和分析代码一次，并共同报告代码可能具有的错误，但每次遇到它们时，解释器都会读取和分析代码语句，并且如果存在某些错误，则会在这方面停止该实例。在实践中，由于现代硬件和先进的编码实践的计算能力改善了所提供的计算能力，因此两者之间的区别是模糊的。  
脚本语言的应用：  
1.在程序中自动执行某些任务  
2.从数据集中提取信息  
3.与传统的编程语言相比，较少的代码密集型  
编程语言的应用：  
1.它们通常在脚本等父节目中运行  
2.与数学模型集成代码的同时更兼容  
3.可以编译像Java这样的语言，然后在任何平台上使用。  
资料来源：互联网  
问候  
Shadab Khokar（软件开发人员）  
### 回答 8
亲爱的大家，  
脚本是膝上型的笔记本电脑程序，甚至小的甚至可以具有复杂性，从而实现效率低效的执行条件。这是长途脚本的情况;这是一个陷入麻烦的脚本。  
例如，如果JavaScript报告与数据库交互，则可能需要几秒钟才能执行。这被PC要求被视为持久的执行时间，因此少数浏览器也可能显示长时间漫步的脚本消息。  
当具有许多代码行的JavaScript报告导致执行时间对浏览器要求过低时，发生了另一个实例。另一个例子是一个不可忽略的javascript文档，直到手动终止它。  
每个Internet浏览器都有它的个人定义了一个长慢跑脚本和演示文稿，当遇到超过浏览器允许执行时间的脚本时的对话框。  
### 回答 9
你好呀，  
脚本是用于自动通过Internet开发人员自动执行任何其他情况下的进程的代码。  
如果使用单一形式的编码语言（如HTML和CSS等标记语言）用于通知计算机以显示Internet网站的方式，用于向程序（如网站）提供命令的脚本语言漫步在电脑上。  
以下是您可能听说过/使用的脚本语言：  
PHP.  
红宝石  
javascript.  
谈到脚本VS编程（在越多标准的义）内时，您不会使用脚本或编步  
你好呀，  
脚本是用于自动通过Internet开发人员自动执行任何其他情况下的进程的代码。  
如果使用单一形式的编码语言（如HTML和CSS等标记语言）用于通知计算机以显示Internet网站的方式，用于向程序（如网站）提供命令的脚本语言漫步在电脑上。  
以下是您可能听说过/使用的脚本语言：  
