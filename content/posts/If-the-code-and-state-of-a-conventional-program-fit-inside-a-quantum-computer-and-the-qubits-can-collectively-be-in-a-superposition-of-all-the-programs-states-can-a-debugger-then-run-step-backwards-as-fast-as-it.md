...
title: 如果传统程序的代码和状态适合量子计算机，并且Qubits可以统称在所有程序状态的叠加中，则可以将调试器尽可能快地运行/步骤返回？
...
### 回答 1
Quantum Computer可以在叠加的思想中，所有[常规]程序的状态是对量子力学的误解和量子计算，不幸的是在Pop-SCI和媒体中经常重复，因此是广泛的误解。  
你需要谈话：  
### 回答 2
不幸的是，由于各种原因，这并不简单。  
...
我不知道在这种情况下调试器意味着什么。  
### 回答 4
是的，除测量外的所有操作都是可逆的。实际上，它比那个更糟糕 - 他们必须是可逆的，这意味着典型代码中的赋值或和/或逻辑运算符不能只是在量子程序中写成，它必须使用辅助Qubits，因为您可以使用'T破坏量子信息（再次，除了通过测量）。这使得经典的令人讨厌有时需要嵌入到极其臃肿的量子程序中，与其经典版本相比。  
...
### 回答 5
简短的答案是没有，但我想知道为什么你会首先结束。听起来你说Quantum Computer可以看出程序可以通过所有可能的状态来看，程序可以并行地从并联，这导致下一个状态并选择出来，但这根本不是量子计算机如何工作。现在，有一个量子算法可以计算名为GROVER算法的任意函数的任意反转，但由于复杂性问题，在这种情况下无法使用。如果它计算从表中的逆，它可能需要扫描旅行中条目数量的平方根，而不是扫描整个表，但是计算机可以进入太多的状态甚至保持平方他们的根在桌子中。  
### 回答 6
我不认为调试器配置为“倒退”。但是您可以录制程序的状态并在许多方面返回它。  
我经常在试图抓住该行为中的错误时做到这一点。没有必要速度向后逐步逐步返回到以前的程序状态。我没有看到Quantum Computer有任何优势。调试量子程序可能是一个痛苦，因为错误可能只出现在一些'路径'中。  
### 回答 7
如果传统程序的代码和状态适合量子计算机，并且Qubits可以统称在所有程序状态的叠加中，则可以将调试器尽可能快地运行/步骤返回？  
...
公共领域只需访问2 Qbit量子技术即可。  
这是可悲的。  
除非我们能够获得对超级效果管道的访问，否则它是每个人自己。  
这并不是说我们中的一些人还没有访问这些。  
### 回答 9
水上的原因  
编程量子计算机与传统计算机的传统编程非常不同，实际上我们必须完全改变我们对计划的理解。  
这是当前阻止我们在写量子计划中的任何进展中都被禁止的这种差异的大小。毕竟，量子模拟器已经存在多年，可以有效地使用它们，以便较少数量的Qubits。现在我们甚至可以免费提供真正的Quantum Computers，可为任何人提供免费（！），并且仍然没有太多。  
目前缺乏行动的原因是了解真正的量子计划的难度。我们肯定理解它们是单一矩阵描述的量子状态的转换，我们甚至知道诸如例如例如例如时的量子计划的步骤。格罗弗的算法以及其他许多算法。  
我们不知道的是这些步骤在古典编程方面的意思。我们还了解所有这些新的量子计划依赖于量子阶段的操纵，但我们没有任何Clue甚至类似于古典计算机的任何东西。  
还有一些关键差异没有谈论，这令人惊讶。观察到量子变换对应于数学中的总函数，从编程透视是始终终止的程序。另一种说法是他们只是有限的国家机器。  
但是有限的状态机已知几十年来不是非常有趣的，因为它们被证明（很多）比图灵机或等同物递归令人令人令人令人难以忍受（RE）函数更强大。这是古典计算，包括练习和理论，一直基于几十年。  
从更传统的程序员实践角度来看，关键问题缺乏循环。 Quantum计算机不能代表最简单的无限环路。在像泵送引理一样简单的事情中，现在在高中或入学级学院CS课程中教授。  
从理论计算机科学的角度来看，量子计划必须终止即将其证明。这意味着每个程序必须具有，明确或隐含地，如此证明。  
现在已经几十年来，将传统的努力转移到推理各个程序被证明是可证明的程序的制造实践中的努力。它具有满足其精确的数学规范的数学证据。这种努力包括功能和逻辑编程，但其他种类也是如此。  
但所有这些努力都基本上无处可行，因为大量的编程拒绝改变并遵守这些原则，合理化，而不是它可以，让我们翼它并写下真正的大型程序，我们稍后可以随时调试。而且它不像他们没有一个点，有很多真正的大程序，这很好地工作。  
但对于量子编程，没有这种合理化。所有量子程序必须根据定义终止。否则他们根本无法表达。  
正如我们已经看到的那样，这种关键差异将具有深远的影响。必须采用和内化用于量子计算以有效地进行。下一步之后将理解量子阶段。  
### 回答 10
十亿q比特你能做什么？您可以将十万位号码分为其素数。这样做会花点时间，10151015cy。每秒有一万亿周期的量子计算机将需要大约十五分钟来定位这个数字。  
但考虑一下。假设我们只有正常的计算机。然后，计算机周期的数量将是exp（（n64/9）.3log（n）2/3）exp（（n64 / 9）.3log（n）2/3）n = 100,000的操作。这是1012210122的操作。  
现在宇宙中有大约10811081个原子。假设我们可以命令宇宙中的每个原子都像超级计算机一样运作，每秒做一万亿的操作。这比每个原子的方式比你的手机快约1000倍。这需要10261026秒然后使用整个宇宙，每个宇宙都像超级计算机一样行为。一年大约有3000万秒钟。因此，我们需要计算3x10183x1018年。  
坏消息：宇宙只有10亿岁。因此，您需要每个atom扮演超级计算机计算不间断的atom，从现在开始到现在，您将以3,000,000,000倍以下的计算机资源缩短。  
然而，单量子电脑只需几分钟就解决了它！  
### 回答 11
即使在发送之前，足以提供您的信息。  
听起来很奇怪吗？让我们以量子计算机的基础。计算机现在使用位来表示可以是0或1.量子计算机使用Qubits，它可以同时使用0,1或两个。是的，量子物理学很奇怪。允许量子颗粒同时称为多个状态的原理称为量子叠加[1]。  
<a> [1] </a>  
量子颗粒的另一个性质是量子缠结[2]，其允许两个缠结的颗粒与它们有多远而传播。那个沟通我  
<a> [2] </a>  
脚注  
即使在发送之前，足以提供您的信息。  
听起来很奇怪吗？让我们以量子计算机的基础。计算机现在使用位来表示可以是0或1.量子计算机使用Qubits，它可以同时使用0,1或两个。是的，量子物理学很奇怪。允许量子颗粒同时称为多个状态的原理称为量子叠加[1]。  
<a> [1] </a>  
量子颗粒的另一个性质是量子缠结[2]，其允许两个缠结的颗粒与它们有多远而传播。并且该沟通比光更快。对真的。等等，我说他们沟通了吗？他们实际上没有。如果测量一个粒子的旋转，则一个方向（例如0状态），那么其他缠绕的旋转将相反（例如1状态）。如果一个改变旋转，另一个人的旋转也会变化。但他们仍然没有沟通。量子理论的魔法。  
<a> [2] </a>  
因此，如果我们使用量子纠缠进行通信，则不再需要物理通信介质，并且通信将比光更快。在这里停下来，足够的量子物理学。  
从你的深入思维中取回相对论理论或壁炉的电影。当你达到光速时，你还记得被教导的时间慢下来。来自理论的另一个陈述是，当您以大于光的速度旅行时，时间率在相反的情况下，这意味着您进入过去。也许现在我可以见到孩子。  
现在将两个理论放在一起，抓住你的思想知道你的留言如何在发送之前送达。  
这个答案是基于我的思想而不是经过验证的事实。如果我错了，欢迎更正。  
图像来源：微软  
脚注  
### 回答 12
理解跨拖波斯的量子计算机的计算能力是误会量子计算。  
它在普遍的科学中常见的是描述量子计算机中的广大州（有50夸张，你得到2 ^ 50 ^ 1中的状态），然后强调这些州可以叠加在一起，为您提供平行的无限潜力的幻觉计算。  
这不是它的工作原理！这不是任何作用的作品！  
Quantum Computer可以形成各州的叠加。所以如果你有2个贵族纪录，你不仅拥有|00‖ |01⟩，|00⟩-12 | 10 + 32 |11⟩，13 |01⟩-23 |11⟩... | 00 + |01⟩，|00⟩-12 | 10 + 32 |11㎡， 13 | 01⟩-23 | 11⟩......等等......  
